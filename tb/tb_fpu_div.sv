...
module tb_fpu_div;
  ...
  // DIV 模块因为涉及长周期的迭代（如 SRT 算法或 Newton-Raphson），
  // 计算周期可能比 FMA 还要长很多，所以将 Timeout 放宽。
  localparam int  TIMEOUT_CYCLES = 1000; 
  localparam string VECTOR_FILE = "F:/git/program/f2/rv32f_fpu/result/div_test_vectors.txt"; 
  ...
  function automatic fpu_operation_type get_op_from_string(string op_name);
    ...
    op = '0;
    if      (op_name == "FDIV")   op.fdiv  = 1'b1;
    else if (op_name == "FSQRT")  op.fsqrt = 1'b1;
    ...
    begin
      ...
      while (top_o.req_ready_o !== 1'b1) begin
        ...
        if (timeout > TIMEOUT_CYCLES) begin
          fail_count++;
          $display("[FAIL] %s -> Request timeout! DIV module might be stalled.", name);
          clear_req();
          return;
        end
      end
      ...
      while (top_o.resp_valid_o !== 1'b1) begin
        ...
        if (timeout > TIMEOUT_CYCLES) begin
          fail_count++;
          $display("[FAIL] %s -> Response timeout! DIV computation takes too long or hung.", name);
          return;
        end
      end
  ...
  // 文件解析与测试驱动任务
  // --------------------------------------------------------------------------
  task automatic run_div_tests_from_file();
    int fd;
    string line;
    int scan_items;
    ...
    begin
      ...
      $display("\n---- RUNNING DIV (DIV/SQRT) TESTS FROM %s ----", VECTOR_FILE);
  ...
  initial begin
    ...
    // 运行基于文本数据的 DIV 测试
    run_div_tests_from_file();

    $display("============================================================");
    $display("DIV TEST SUMMARY: PASS=%0d FAIL=%0d", pass_count, fail_count);
    $display("============================================================");

    if (fail_count == 0) begin
      $display("ALL DIV TESTS PASSED SUCCESSFULLY");
    end else begin
      $display("WARNING: SOME DIV TESTS FAILED!");
    end
