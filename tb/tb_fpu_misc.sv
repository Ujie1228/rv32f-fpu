...
module tb_fpu_misc;
  ...
  localparam string VECTOR_FILE = "F:/git/program/f2/rv32f_fpu/result/misc_test_vectors.txt";
  ...
  function automatic fpu_operation_type get_op_from_string(string op_name);
    ...
    op = '0;
    if (op_name == "FSGNJ")       op.fsgnj  = 1'b1;
    else if (op_name == "FCMP")   op.fcmp   = 1'b1;
    else if (op_name == "FMAX")   op.fmax   = 1'b1;
    else if (op_name == "FCLASS") op.fclass = 1'b1;
  ...
  // 文件解析与测试驱动任务
  // --------------------------------------------------------------------------
  task automatic run_tests_from_file();
    int fd;
    string line;
    int scan_items;
    ...
    begin
      ...
      $display("\n---- RUNNING MISC TESTS FROM %s ----", VECTOR_FILE);
